...
if __name__ == "__main__":
    """
    Main execution pipeline for training and evaluating CLAReSNet.
    
    Pipeline:
        1. Setup device and configuration
        2. Load and preprocess data (PCA dimensionality reduction)
    ...
    PRETRAINED_PATH = "claresnet_indianpines_pretrained.pth"
    ...
    ind_spectral, ind_label_img = load_dataset_from_csv(
        "IndianPines.csv", 145, 145
    )
    
    # Apply PCA dimensionality reduction
    print(f"Applying PCA: {ind_spectral.shape[-1]} -> {n_components} bands")
    pca_ind = PCA(n_components=n_components)
    ind_reduced = pca_ind.fit_transform(
        ind_spectral.reshape(-1, ind_spectral.shape[-1])
    )
    ind_reduced = ind_reduced.reshape(
        ind_spectral.shape[0], ind_spectral.shape[1], n_components
    )
    
    # Get labeled samples
    ind_samples = get_labeled_samples(ind_label_img)
    print(f"Total labeled samples: {len(ind_samples)}")
    
    # Create train/val/test splits
    # 80% train+val, 20% test
    train_val, test_ind = train_test_split(
        ind_samples, test_size=0.2,
        stratify=ind_samples['label'],
        random_state=42
    )
    
    # Split train+val into 72% train, 8% val
    ...
    train_loader_ind = DataLoader(
        HyperspectralDataset(ind_reduced, train_ind, patch_size, augment=True),
        batch_size=16, shuffle=True
    )
    val_loader_ind = DataLoader(
        HyperspectralDataset(ind_reduced, val_ind, patch_size),
        batch_size=32, shuffle=False
    )
    test_loader_ind = DataLoader(
        HyperspectralDataset(ind_reduced, test_ind, patch_size),
        batch_size=32, shuffle=False
    )
    ...
    for epoch in range(num_epochs):
        ...
        train_loss, train_acc = train_one_epoch(
            model, train_loader_ind, optimizer, criterion, device,
            desc=f"Epoch {epoch+1}/{num_epochs}"
        )
        
        # Validate
        val_acc, _, _, val_loss = evaluate(
            model, val_loader_ind, criterion, device, desc="Validation"
        )
        
        # Update tracker
        ind_tracker.update(epoch + 1, train_loss, train_acc, val_loss, val_acc)
    ...
    ind_tracker.plot_training_curves(title="Indian Pines Training")
    ...
    ind_metrics = comprehensive_evaluation(
        model=model,
        loader=test_loader_ind,
        device=device,
        dataset_name="Indian Pines",
        class_names=indian_class_names,
        spectral_image=ind_reduced,
        label_image=ind_label_img,
        samples_df=test_ind,
    ...
    print("="*50)
    print(f"Final Test Accuracy: {ind_metrics['accuracy']*100:.2f}%")
    print(f"Model saved to: {PRETRAINED_PATH}")


"""
=============================================================================
USAGE EXAMPLE AND DIMENSION TRACKING
=============================================================================

Input Data Dimensions:
----------------------
- Raw hyperspectral image: (H, W, C_original)
  Example: Indian Pines (145, 145, 200)

- After PCA reduction: (H, W, C_reduced)
  Example: (145, 145, 30)

- Labeled pixels DataFrame: (N_samples, 3) columns: [row, col, label]
  Example: (10249, 3) for Indian Pines

- Training batch: (B, C_reduced, patch_size, patch_size)
  Example: (16, 30, 11, 11)
...
Evaluation Outputs:
-------------------
- Confusion matrix: (n_classes, n_classes) = (16, 16)
- Per-class metrics: (n_classes,) = (16,)
- t-SNE features: (N_samples, 2) for 2D visualization
- PCA features: (N_samples, 2) for 2D visualization
- Classification map: (H, W) = (145, 145) with predicted labels
- Uncertainty map: (H, W) = (145, 145) with entropy values
